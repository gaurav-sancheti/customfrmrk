...
def test_get_company_brand_for_non_existing_company(company_service_rest_api, access_token_with_company_read_scope):
    random_company_id = generate_uuid()

    response, parsed_body = company_service_rest_api.get_company_brand(
        company_id=random_company_id, access_token=access_token_with_company_read_scope)
...
def test_get_company_brand_for_non_existing_brand(
...
def test_get_company_brand_without_access_token(company_service_rest_api, env_data):
    company_id = env_data['companies']['cc_company_service_test_company']['id']

    response, parsed_body = company_service_rest_api.get_company_brand(company_id=company_id, access_token=None)
...
def test_get_company_brand_with_invalid_token(
...
def test_get_company_brand_without_required_scope(
